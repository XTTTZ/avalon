...
| 集合           | 服务端内容                                  | 到期时间                                 |
| -------------- | ------------------------------------------- | ---------------------------------------- |
| `avalon_rooms` | 权威房间状态、身份、秘密票、命令去重记录    | 大厅闲置 24h / 局中闲置 48h / 结束后 24h |
| `avalon_users` | 随机化玩家标识、上次房间、建房/加入去重记录 | 365 天；登录接近到期时续期               |
| `avalon_notes` | 房间 + 可信会话玩家自己的昵称、猜测和笔记   | 绑定房间实例，随活跃房间保留             |
...
为该函数添加定时触发器，名称必须为 **`avalon-cleanup`**，建议每小时触发一次（按控制台所示时区设置）。平台事件 `Type=Timer`、`TriggerName=avalon-cleanup` 触发清理；HTTP 请求体中的同名字段不会被视为平台触发器。每次最多处理 400 条过期记录，对每条在事务中再次确认过期。大厅无操作 24 小时、局中无操作 48 小时、结束后固定 24 小时到期；最后一人离开大厅立即到期。轮询和笔记保存不刷新房间期限。尚未清理的过期房间也不能继续使用。

清理到笔记时，在同一事务内核对所属房间实例、成员资格和最新期限：房间仍活跃则保留笔记并更新笔记期限，否则删除到期笔记。`instanceId` 在建房时生成，重新开局不变，四位房号复用时必定不同；旧格式房间继续按原期限及创建时间兼容。PostgreSQL 清理 RPC 同时锁定候选记录及相关房间，使用与写事务相同的排序锁，避免续期竞争误删。

**从旧版本升级必须先重新执行本版 `postgres.sql`，再发布云函数**，以更新 `avalon_cleanup`。这是保留数据的函数更新，不是重建数据库。CI 会在独立 PostgreSQL 17 测试库执行 `tests/postgres-cleanup.sql`；该脚本仅允许数据库名 `avalon_test`，不要在生产执行测试脚本。
